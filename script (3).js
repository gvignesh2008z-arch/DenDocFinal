...
if (servicesGrid) {
  SERVICES.forEach((s,i)=>{
    ...
    el.innerHTML = `
      ...
      <h3>${s.title}</h3>
      <p>${s.desc}</p>
      <a href="contact.html#appointment" class="service-link">Learn More
  ...
  DOCTORS.forEach((d,i)=>{
    ...
    el.innerHTML = `
      ...
      <div class="doctor-info">
        ...
        <p>${d.exp}</p>
        <a href="contact.html#appointment" class="doctor-book">Book Consultation</a>
      </div>`;
    doctorsGrid.appendChild(el);
  });
}

/* ============================= BUILD: TECH (services.html) ============================= */
...
if (testSlides && testDots) {
  ...
  if (testPrev) testPrev.addEventListener('click', ()=>goTestimonial(testIndex-1));
  if (testNext) testNext.addEventListener('click', ()=>goTestimonial(testIndex+1));
  [...testDots.children].forEach(d=>d.addEventListener('click', ()=>goTestimonial(+d.dataset.i)));
  setInterval(()=>goTestimonial(testIndex+1), 6000);
}

/* ============================= BUILD: FAQ (contact.html) ============================= */
...
/* ============================= BUILD: BEFORE/AFTER GALLERY (about.html) ============================= */
...
if (navLinks) {
  document.querySelectorAll('.nav-links a').forEach(a=>a.addEventListener('click', ()=>{
    ...
    if (mobileToggle) mobileToggle.innerHTML = iconSVG('menu');
    if (window.lucide) lucide.createIcons();
  }));
}
